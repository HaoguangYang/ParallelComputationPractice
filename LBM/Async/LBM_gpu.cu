         ...
         bool save, unsigned int NY1)
{
    ...
    if(save) {
        checkCudaErrors(cudaMemcpyAsync(f0, f0_gpu, size_scalar_data, cudaMemcpyDeviceToHost, S0));
        checkCudaErrors(cudaMemcpy(f2, f2_gpu, size_field_data, cudaMemcpyDeviceToHost));
        checkCudaErrors(cudaMemcpyAsync(r, rho_gpu, size_scalar_data, cudaMemcpyDeviceToHost, S1));
        checkCudaErrors(cudaMemcpyAsync(u, ux_gpu, size_scalar_data, cudaMemcpyDeviceToHost, S2));
        checkCudaErrors(cudaMemcpyAsync(v, uy_gpu, size_scalar_data, cudaMemcpyDeviceToHost, S3));
    }
    else {
        ...
        checkCudaErrors(cudaMemcpyAsync(&f2[NX*(ndir-1)*(NY1-1)], &f2_gpu[NX*(ndir-1)*(NY1-1)], NX*sizeof(double)*(ndir-1), cudaMemcpyDeviceToHost, S0));
        //checkCudaErrors(cudaMemcpy(f2, f2_gpu, size_field_data, cudaMemcpyDeviceToHost));
    }
...
__host__ void CUDAAsyncLoad (double *f1, unsigned int NY1){
    cudaStream_t S0, S1;
    checkCudaErrors(cudaMemcpyAsync(&f1_gpu[NX*(ndir-1)], &f1[NX*(ndir-1)], NX*(ndir-1)*sizeof(double), cudaMemcpyHostToDevice, S0));
    checkCudaErrors(cudaMemcpyAsync(&(f1_gpu[NX*(ndir-1)*NY1]), &(f1[NX*(ndir-1)*NY1]), NX*(ndir-1)*sizeof(double), cudaMemcpyHostToDevice,S1));
}

__host__ void CUDASync() {cudaDeviceSynchronize();}
