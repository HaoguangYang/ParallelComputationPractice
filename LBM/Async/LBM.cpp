...
{
    #pragma omp parallel for shared(t,r,u,v) schedule(guided)
...
{
    #pragma omp parallel for shared(f0,f1,r,u,v) schedule(guided)
    for(unsigned int y = 0; y < ny; ++y)
    {
        for(unsigned int x = 0; x < NX; ++x)
        {
...
void stream_collide_save(double *f0, double *f1, double *f2, double *r, double *u, double *v, bool save, unsigned int ystart, unsigned int yend)
{
    ...
    {
        #pragma omp parallel for shared(f0,f1,f2,r,u,v,save) schedule(guided)
        for(unsigned int x = 0; x < NX; ++x)
        {
...
void stream_collide_save_internal(double *f0, double *f1, double *f2, double *r, double *u, double *v, bool save, unsigned int ystart, unsigned int rank_ny, int nr, MPI_Request *reqs, MPI_Status *stats)
{
    ...
    stream_collide_save_wrapper(f0, f1, f2, r, u, v, save, rank_ny);
...
void compute_flow_properties(unsigned int t, double *r, double *u, double *v, double *prop, int rank, unsigned int ystart, unsigned int ny)
{
    ...
    local_sumdata[0] = E;
    local_sumdata[1] = sumrhoe2;
    local_sumdata[2] = sumuxe2;
    local_sumdata[3] = sumuye2;
    local_sumdata[4] = sumrhoa2;
    local_sumdata[5] = sumuxa2;
    local_sumdata[6] = sumuya2;
