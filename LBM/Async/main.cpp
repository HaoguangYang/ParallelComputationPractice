...
int main(int argc, char* argv[])
{
    ...
    if(rank == 0)
    {
        int maxThreadsDim[3];
        int maxGridSize[3];
        int maxThrdPerGPU, deviceCount;
        devQuery(maxThrdPerGPU, deviceCount, maxThreadsDim, maxGridSize);
    ...
    //double *swapy0 = (double*) malloc(sizeof(double)*NX*ndir);
    //double *swapy1 = (double*) malloc(sizeof(double)*NX*ndir);
    //size_t mem_size_n0dir1  = sizeof(double)*NX*(rank_ny)*(ndir-1);
    deviceSetup (mem_size_0dir, mem_size_n0dir, mem_size_scalar);
    ...
    for(unsigned int n = 0; n < NSTEPS; ++n)
    {
        ...
        CUDASync();
        stream_collide_save_internal(f0,f1,f2,rho,ux,uy,need_scalars,1,rank_ny, 4,reqs,stats);
        /*for (int x = 0; x < NX; x++){
            for (int y = 1; y < ndir; y++){
                f2[fieldn_index(x,1,y)] = swapy0[fieldn_index(x,-1,y)];
                f2[fieldn_index(x,rank_ny-1,y)] = swapy1[fieldn_index(x,-1,y)];
            }
            f0[scalar_index(x,1)] = swapy0[fieldn_index(0,0,x+1)];
            f0[scalar_index(x,rank_ny-1)] = swapy1[fieldn_index(0,0,x+1)];
        }*/
