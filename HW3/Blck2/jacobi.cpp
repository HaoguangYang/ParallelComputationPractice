...
int main(int argc, char* argv[]) {
	...
	if (maxm<maxn || maxm<maxp) MPI_GS1 = 0;
	else MPI_GS1 = floor(maxm/unit_length);
	if (maxn<maxm || maxn<maxp) MPI_GS2 = 0;
	else MPI_GS2 = floor(maxn/unit_length);
	if (maxp<maxm || maxp<maxn) MPI_GS3 = 0;
	else MPI_GS3 = floor(maxp/unit_length);
	if (MPI_GS1!=0 && MPI_GS2!=0 && MPI_GS3!=0) MPI_GS3 = 0;
	if (MPI_GS1 == 0){
		MPI_GS1 = 1;
		if (MPI_GS2 == 0){
			MPI_GS2 = 1;
